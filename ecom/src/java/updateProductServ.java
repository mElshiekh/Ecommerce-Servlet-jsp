...
@WebServlet(urlPatterns = {"/updateProductServ"})
public class updateProductServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!pp.find()){
            session.setAttribute("invupd", "quant");
            response.sendRedirect("EditProductServ?prodId="+productid);
        }
        else if(!pq.find()){
            session.setAttribute("invupd", "price");
            response.sendRedirect("EditProductServ?prodId="+productid);
        }
        else{
